...
$user = new User($db, $set->users_table);

// we check for cookies to autologin
if(!$user->islg() && isset($_COOKIE['user']) && isset($_COOKIE['pass'])) {
	 if(($usr = $db->get_row("SELECT `userid` FROM `$set->users_table` WHERE `username` = '".$db->escape($_COOKIE['user'])."' AND `password` = '".$db->escape($_COOKIE['pass'])."'")))
